...
export default class StudentSignupsByTypeBarChart extends Component {
  constructor(props) {
    super(props)
  ...
  addBarChartData(events) {
    ...
    for (let event in events) {
      if (studentTypeCount[events[event].type]===undefined) {
        studentTypeCount[events[event].type] = 0;
      }

      for (let timeslot in events[event].timeslots){
        if(events[event].timeslots[timeslot].student1){
          studentTypeCount[events[event].type] += 1;
        }
    ...
    const svg = d3.select('.studentSignupsByTypeBarChart')
  ...
  render() {
    return (
      <div className="studentSignupsByTypeBarChartContainer barChartContainer" style={divStyle}>
        <svg className="studentSignupsByTypeBarChart">
        </svg>
      </div>
    );
  }
