...
export default class StudentSignupsByLocationBarChart extends Component {
  constructor(props) {
    super(props)
  ...
  addBarChartData(events) {
    let studentTypeCount = {};

    for (let event in events) {
      if (studentTypeCount[events[event].location]===undefined) {
        studentTypeCount[events[event].location] = 0;
      }

      for (let timeslot in events[event].timeslots){
        if(events[event].timeslots[timeslot].student1){
          studentTypeCount[events[event].location] += 1;
        }
    ...
    const svg = d3.select('.studentSignupsByLocationBarChart')
    ...
    const data = d3.entries(studentTypeCount).sort((a, b) => ( b.value - a.value ));
  ...
  render() {
    return (
      <div className="studentSignupsByLocationBarChartContainer barChartContainer" style={divStyle}>
        <svg className="studentSignupsByLocationBarChart">
        </svg>
      </div>
    );
  }
