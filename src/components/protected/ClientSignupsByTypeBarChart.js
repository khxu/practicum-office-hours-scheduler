...
export default class ClientSignupsByTypeBarChart extends Component {
  constructor(props) {
    super(props)
  ...
  addBarChartData(events) {
    ...
    for (let event in events) {
      if (clientTypeCount[events[event].type]===undefined) {
        clientTypeCount[events[event].type] = 0;
      }

      for (let timeslot in events[event].timeslots){
        if(events[event].timeslots[timeslot].client){
          clientTypeCount[events[event].type] += 1;
        }
      }
    }
    
    const svg = d3.select('.clientSignupsByTypeBarChart')
    ...
    const data = d3.entries(clientTypeCount).sort((a, b) => ( b.value - a.value ));
  ...
  render() {
    return (
      <div className="clientSignupsByTypeBarChartContainer barChartContainer" style={divStyle}>
        <svg className="clientSignupsByTypeBarChart">
        </svg>
      </div>
    );
  }
